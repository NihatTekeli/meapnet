...
#v1,v2,v3,...vN,r1,r2,r3,...rN to v1,r1,v2,r2,v3,r3,...vN,rN
def RestructureCrossModalFeatures(x, num_id, feat_dims):
    B = x.shape[0]
    x1 = x[:B//2].reshape((num_id, feat_dims,-1))
    x2 = x[B//2:].reshape((num_id, feat_dims,-1))
    x_ordered = torch.cat((x1, x2), 1).reshape((B,-1))
    return x_ordered

class MeapLoss(torch.nn.Module):
    ...
    def __init__(self, anneal, batch_size, num_id, feat_dims, margin_i, margin_x):
        """
        Parameters
        ----------
        anneal : float
    ...
    def forward(self, preds):
        ...
        #Reorder visible and infrared features
        preds = RestructureCrossModalFeatures(preds, self.num_id, self.feat_dims)
        ...
        #Apply margin terms to full sample set
        group = int(self.batch_size / self.num_id)
        pos_mask = torch.zeros(self.batch_size,self.batch_size) 
        cross_mask = torch.zeros(self.batch_size,self.batch_size) 
        for ind in range(self.num_id):
            pos_mask[(ind * group):((ind + 1) * group), (ind * group):((ind + 1) * group)] = 1
            cross_mask[(ind * group):((ind + 1) * group - group//2), (ind * group + group//2):((ind + 1) * group)] = 1
            cross_mask[(ind * group + group//2) :((ind + 1) * group), (ind * group):((ind + 1) * group - group//2)] = 1
        sim_all = sim_all - self.margin_i * pos_mask.cuda() - self.margin_x * cross_mask.cuda()
        ...
        #Apply margin terms to positive sample set
        group = int(self.batch_size / self.num_id)
        cross_pos_mask = torch.zeros(self.num_id, group, group)
        cross_pos_mask[:, 0:group//2, group//2:] = 1
        cross_pos_mask[:, group//2:, 0:group//2] = 1
        sim_pos = sim_pos - self.margin_i - self.margin_x * cross_pos_mask.cuda()
