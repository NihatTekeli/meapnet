...
def extract_gall_feat(gall_loader):
    ...
    start = time.time()
    ptr = 0
    gall_feat_pool = np.zeros((ngall, net.total_pool_dim))
    gall_feat_fc = np.zeros((ngall, net.total_pool_dim))
    with torch.no_grad():
...
def extract_query_feat(query_loader):
    ...
    start = time.time()
    ptr = 0
    query_feat_pool = np.zeros((nquery, net.total_pool_dim))
    query_feat_fc = np.zeros((nquery, net.total_pool_dim))
    with torch.no_grad():
